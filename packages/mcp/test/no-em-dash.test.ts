 ...
 * the product. Here that means every string an agent gets back from the server: tool titles and
 * descriptions, the JSON Schema descriptions of their arguments, resource descriptions, prompt
 * texts, error messages. This suite reads the source through the TypeScript parser rather than
 * with a regular expression, because the parser is what tells a string from a comment, and
 * `/'/g` from the start of one: what is checked here is the visible text. Comments are held to
 * the same rule by `packages/shared/test/public-sources.test.ts`, which walks the whole tree.
 */
